...
table.p.value.pre <- p.value.pre %>%
  mutate(variable = factor(variable, levels = c("n", "age", "height", "total_mass", "bmi", "fat_mass", "lean_mass", "sys", "dia", "triglyserider", "hdl", "ldl", "glukose", "pwv", "vo2max", "wc", "vat", "handgrip", "rm"),
                           labels = c("N", "Alder (år)", "Høyde (cm)", "Vekt (kg)", "BMI (kg/m2)", "Fettmasse (kg)", "Mager masse (kg)", "Systolisk blodtrykk (mmHg)", "Diastolisk blodtrykk (mmHg)", "Midjeomkrets (cm)", "Visceralt fett (g)", "Triglyserider (mmol/l)", "HDL-kolesterol (mmol/l)", "LDL-kolesterol (mmol/l)", "Glukose (mmol/l)", "Pulsbølgehastighet (m/s)", "VO2maks (ml/min/kg)", "Gripestyrke (kg)", "Benstyrke 1RM (kg)"))) %>%
...
table.baselinechar001 <- merge(data.frame(table.baselinechar2, row.names = NULL), data.frame(table.p.value.pre, row.names = NULL), by = 0, all = TRUE) [-1] %>%
  ...
  flextable::save_as_image(path = "baseline_table.png")
  print()
