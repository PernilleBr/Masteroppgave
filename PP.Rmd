...
mean.pp.vat %>%
  ggplot(aes(group, mean.vat.pros, group = group)) + geom_point(size = 3, aes(group = group)) + geom_point(data = pp.vat.stat, aes(group, change.pros, group = id), position = position_dodge(width = 0.2), size = 2, alpha = 0.4) + geom_errorbar(aes(ymin = mean.vat.pros - sd.vat.pros, ymax = mean.vat.pros + sd.vat.pros), position = position_dodge(width = 0.2), width = 0.05) + theme_classic() + labs(x = "Gruppe", y = "Visceralt fett (% endring)")
```
```{r}
library(tidyverse); library(knitr); library(ggplot2); library(kableExtra); library(readxl); library(dplyr); library(flextable); library(pbkrtest); library(broom)
Resultatskjema <- read_excel("Resultatskjema.xlsx")
