...
mean.itt.gluk <- itt.gluk %>%
  drop_na() %>%
  summarise(mean.gluk = mean(change.gluk.itt),
            sd.gluk = sd(change.gluk.itt)) %>%
  print()
...
figur.gluk.itt <- mean.itt.gluk %>%
  ggplot(aes(group, mean.gluk, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.gluk - sd.gluk, ymax = mean.gluk + sd.gluk), width = 0.1) + theme_classic() + geom_point(data = dropout.itt.gluk, aes(group, change.gluk.itt), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i glukose (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-3.2, 1.1)
...
mean.pp.gluk <- pp.gluk %>%
  summarise(mean.gluk.pp = mean(change.gluk.pp),
            sd.gluk.pp = sd(change.gluk.pp)) %>%
  print()

figur.gluk.pp <- mean.pp.gluk %>%
  ggplot(aes(group, mean.gluk.pp, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.gluk.pp - sd.gluk.pp, ymax = mean.gluk.pp + sd.gluk.pp), width = 0.1) + theme_classic() + geom_point(data = pp.gluk, aes(group, change.gluk.pp), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i glukose (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-3.2, 1.1)
...
mean.ppm.gluk <- ppm.gluk %>%
  summarise(mean.gluk.ppm = mean(change.gluk.ppm),
            sd.gluk.ppm = sd(change.gluk.ppm)) %>%
  print()

figur.gluk.ppm <- mean.ppm.gluk %>%
  ggplot(aes(group, mean.gluk.ppm, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.gluk.ppm - sd.gluk.ppm, ymax = mean.gluk.ppm + sd.gluk.ppm), width = 0.1) + theme_classic() + geom_point(data = ppm.gluk, aes(group, change.gluk.ppm), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i glukose (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-3.2, 1.1)
