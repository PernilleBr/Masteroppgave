...
mean.itt.tg <- itt.tg %>%
  drop_na() %>%
  summarise(mean.tg = mean(change.tg.itt),
            sd.tg = sd(change.tg.itt)) %>%
  print()
...
figur.tg.itt <- mean.itt.tg %>%
  ggplot(aes(group, mean.tg, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.tg - sd.tg, ymax = mean.tg + sd.tg), width = 0.1) + theme_classic() + geom_point(data = dropout.itt.tg, aes(group, change.tg.itt), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i triglyserider (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-1.2, 0.8)
...
mean.pp.tg <- pp.tg %>%
  summarise(mean.tg.pp = mean(change.tg.pp),
            sd.tg.pp = sd(change.tg.pp)) %>%
  print()

figur.tg.pp <- mean.pp.tg %>%
  ggplot(aes(group, mean.tg.pp, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.tg.pp - sd.tg.pp, ymax = mean.tg.pp + sd.tg.pp), width = 0.1) + theme_classic() + geom_point(data = pp.tg, aes(group, change.tg.pp), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i triglyserider (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-1.2, 0.8)
...
# lager et datasett med gjennomsnittsverdier av prosentvis endring for bruk i graf
mean.ppm.tg <- ppm.tg %>%
  summarise(mean.tg.ppm = mean(change.tg.ppm),
            sd.tg.ppm = sd(change.tg.ppm)) %>%
  print()

figur.tg.ppm <- mean.ppm.tg %>%
  ggplot(aes(group, mean.tg.ppm, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.tg.ppm - sd.tg.ppm, ymax = mean.tg.ppm + sd.tg.ppm), width = 0.1) + theme_classic() + geom_point(data = ppm.tg, aes(group, change.tg.ppm), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i triglyserider (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-1.2, 0.8)
...
mean.itt.hdl <- itt.hdl %>%
  drop_na() %>%
  summarise(mean.hdl = mean(change.hdl.itt),
            sd.hdl = sd(change.hdl.itt)) %>%
  print()
...
figur.hdl.itt <- mean.itt.hdl %>%
  ggplot(aes(group, mean.hdl, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.hdl - sd.hdl, ymax = mean.hdl + sd.hdl), width = 0.1) + theme_classic() + geom_point(data = dropout.itt.hdl, aes(group, change.hdl.itt), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i HDL-kolesterol (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-0.45, 0.4)
...
# lager et datasett med gjennomsnittsverdier av prosentvis endring for bruk i graf
mean.pp.hdl <- pp.hdl %>%
  summarise(mean.hdl.pp = mean(change.hdl.pp),
            sd.hdl.pp = sd(change.hdl.pp)) %>%
  print()

figur.hdl.pp <- mean.pp.hdl %>%
  ggplot(aes(group, mean.hdl.pp, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.hdl.pp - sd.hdl.pp, ymax = mean.hdl.pp + sd.hdl.pp), width = 0.1) + theme_classic() + geom_point(data = pp.hdl, aes(group, change.hdl.pp), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i HDL-kolesterol (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-0.45, 0.4)
...
mean.ppm.hdl <- ppm.hdl %>%
  summarise(mean.hdl.ppm = mean(change.hdl.ppm),
            sd.hdl.ppm = sd(change.hdl.ppm)) %>%
  print()

figur.hdl.ppm <- mean.ppm.hdl %>%
  ggplot(aes(group, mean.hdl.ppm, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.hdl.ppm - sd.hdl.ppm, ymax = mean.hdl.ppm + sd.hdl.ppm), width = 0.1) + theme_classic() + geom_point(data = ppm.hdl, aes(group, change.hdl.ppm), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i HDL-koleserol (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-0.45, 0.4)
...
mean.itt.ldl <- itt.ldl %>%
  drop_na() %>%
  summarise(mean.ldl = mean(change.ldl.itt),
            sd.ldl = sd(change.ldl.itt)) %>%
  print()
...
figur.ldl.itt <- mean.itt.ldl %>%
  ggplot(aes(group, mean.ldl, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.ldl - sd.ldl, ymax = mean.ldl + sd.ldl), width = 0.1) + theme_classic() + geom_point(data = dropout.itt.ldl, aes(group, change.ldl.itt), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i LDL-kolesterol (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-0.5, 2)
...
# lager et datasett med gjennomsnittsverdier av prosentvis endring for bruk i graf
mean.pp.ldl <- pp.ldl %>%
  summarise(mean.ldl.pp = mean(change.ldl.pp),
            sd.ldl.pp = sd(change.ldl.pp)) %>%
  print()

figur.ldl.pp <- mean.pp.ldl %>%
  ggplot(aes(group, mean.ldl.pp, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.ldl.pp - sd.ldl.pp, ymax = mean.ldl.pp + sd.ldl.pp), width = 0.1) + theme_classic() + geom_point(data = pp.ldl, aes(group, change.ldl.pp), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i LDL-kolesterol (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-0.5, 2)
...
mean.ppm.ldl <- ppm.ldl %>%
  summarise(mean.ldl.ppm = mean(change.ldl.ppm),
            sd.ldl.ppm = sd(change.ldl.ppm)) %>%
  print()

figur.ldl.ppm <- mean.ppm.ldl %>%
  ggplot(aes(group, mean.ldl.ppm, group = group)) + geom_boxplot(width = 0.3, fill = "white", colour = "black") + geom_errorbar(aes(ymin = mean.ldl.ppm - sd.ldl.ppm, ymax = mean.ldl.ppm + sd.ldl.ppm), width = 0.1) + theme_classic() + geom_point(data = ppm.ldl, aes(group, change.ldl.ppm), size = 2, alpha = 0.4) + labs(x = "Gruppe", y = "Endring i LDL-koleserol (mmol/l)") + geom_hline(yintercept = 0, linetype = "solid", color = "black") + theme(axis.title = element_text(size = 11), axis.text.x = element_text(size = 10), axis.text.y = element_text(size = 10)) + ylim(-0.5, 2)
