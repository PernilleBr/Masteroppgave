...
pre.diff <- Resultatskjema %>%
  ...
  select(id, group, time, sex, age, total_mass, height, weight, bmi, fat_mass, lean_mass, sys, dia, wc, handgrip, vo2max_ml_min_kg_6x5, vat_mass, vat_volume, `s-glukose`, `s-triglyserider`, `s-hdl`, `s-ldl`, pwv, `1rm_newton`, phys_act) %>% 
...
t.test(age ~ group, data = pre.diff, var.equal = TRUE)
...
pre.diff.women <- Resultatskjema %>%
  # velger relevante variabler
  select(id, group, time, sex, age, height, weight, total_mass, bmi, fat_mass, lean_mass, sys, dia, wc, handgrip, vo2max_ml_min_kg_6x5, vat_mass, vat_volume, `s-glukose`, `s-triglyserider`, `s-hdl`, `s-ldl`, pwv, `1rm_newton`, phys_act) %>% 
  # filtrerer verdier fra pretest
  filter(time == "pre") %>%
  filter(sex == "1") %>%
